...
unsigned int dataMemory[MAX_INSTRUCTIONS];
...
void parseInstruction(char *line, int address) {
    ...
    if (strncmp(line, ".word", 5) == 0) {
        ...
        if (token != NULL) {
            dataMemory[dataMemorySize++] = (unsigned int)strtoul(token, NULL, 0);
        }
    } else {
...
void secondPass(FILE *imeminFile, FILE *dmeminFile) {
    ...
    // Write data memory to dmemin.txt
